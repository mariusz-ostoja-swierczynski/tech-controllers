...
class TechTilesLastUpdateSensor(CoordinatorEntity, SensorEntity):
    ...
    def __init__(self, coordinator: TechCoordinator, config_entry: ConfigEntry) -> None:
        ...
        self._attr_translation_placeholders = {"entity_name": config_entry.title}
    ...
    def name(self) -> str | UndefinedType | None:
        """Use the translation key for the entity name."""
        return None
...
class TechOutsideTempTile(CoordinatorEntity, SensorEntity):
    ...
    def _handle_coordinator_update(self, *args: Any) -> None:
        """Handle updated data from the coordinator."""
        device = self._coordinator.data["tiles"].get(self._id)
        if device is None:
            self._attr_available = False
            self.async_write_ha_state()
            return
...
class TileOpenThermSensor(TileSensor, SensorEntity):
    ...
    def __init__(
        ...
        if self._txt_id is not None:
            ...
            self._attr_translation_placeholders = {"entity_name": base_name}
            self._name = None
